...
/*
  Yet another instance of bound propagation.
*/
...
/*
  If there is a row with a singleton column such that no matter what the
  values of the other variables are, the constraint forces the singleton
  ...
  (assuming the constraint is not trivial), since the variables do not
  imply an upper or lower bound.
  ...
  There is nothing to prevent us from substituting away a variable in an
  equality from the other constraints it appears in, but since that causes
  fill-in, it wouldn't make sense unless we could then drop the equality
  itself.  We can't do that if the bounds on the variable in equation aren't
  implied by the equality.  Another way of thinking of this is that there
  is nothing special about an equality; just like one can't always drop
  an inequality constraint with a column singleton, one can't always drop
  an equality.
    ...
    CoinPresolveMatrix *prob, const CoinPresolveAction *next, int &fill_level)
{
  ...
  const unsigned char *integerType = prob->integerType_ ;
  /*
    Documented as `inhibit x+y+z = 1 mods'. Unclear at a quick glance just how
    that applies below. The usual complete lack of comment.  -- lh, 110528 --
  */
  bool stopSomeStuff = ((prob->presolveOptions()&0x04) != 0) ;
