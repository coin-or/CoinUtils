...
void dbg_find_elem(const CoinPostsolveMatrix *postMtx, int i, int j)
{
  ...
  CoinBigIndex krow = presolve_find_row3(i, kcs, lenj, postMtx->hrow_, postMtx->link_);
  if (krow >= 0) {
    std::cout
...
*/
  ...
  for (CoinBigIndex krowy = krsy; krowy < krey; krowy++) {
    ...
    while (krowx < krex0 && hcol[krowx] < j)
      krowx++;
...
*/
    for (int j = 0; j < ncols; ++j) {
      if (colLengths[j] > 0 && cdone[j]) {
        ...
        CoinBigIndex krow = presolve_find_row3(tgtrow, kcs, lenj, rowIndices, link);
        if (krow >= 0) {
          std::cout
