...
*/
    /* Test changed to use +small tolerance rather than -tolerance
       as test fails often */
    if (((rlo[irow] <= -PRESOLVE_INF) || (-PRESOLVE_INF < maxdown && rlo[irow] <= maxdown + inftol2)) && ((rup[irow] >= PRESOLVE_INF) || (maxup < PRESOLVE_INF && rup[irow] >= maxup - inftol2))) {
...
*/
    ...
    for (int k = 0; k < nlo; k++) {
      ...
      if (change && prob->getColumnStatus(jcol) != CoinPrePostsolveMatrix::basic) {
        prob->setColumnStatus(jcol, CoinPrePostsolveMatrix::atLowerBound);
        if (cbarj < -ztoldj || clo[jcol] <= -COIN_DBL_MAX)
          dualfeas = false;
      }
      cup[jcol] = bounds[k];
...
*/
    for (int k = nlo; k < ninrow; k++) {
      ...
      if (change && prob->getColumnStatus(jcol) != CoinPrePostsolveMatrix::basic) {
        ...
        if (cbarj > ztoldj || cup[jcol] >= COIN_DBL_MAX)
          dualfeas = false;
      }
      clo[jcol] = bounds[k];
