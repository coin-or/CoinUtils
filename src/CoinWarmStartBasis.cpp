...
void CoinWarmStartBasis::setSize(int ns, int na)
{
...
void CoinWarmStartBasis::assignBasisStatus(int ns, int na, char *&sStat,
  char *&aStat)
{
  // Round all so arrays multiple of 4
  int nintS = (ns + 15) >> 4;
...
CoinWarmStartBasis::CoinWarmStartBasis(const CoinWarmStartBasis &ws)
  : numStructural_(ws.numStructural_)
  , numArtificial_(ws.numArtificial_)
  , structuralStatus_(NULL)
  , artificialStatus_(NULL)
{
  // Round all so arrays multiple of 4
...
// Resizes
void CoinWarmStartBasis::resize(int newNumberRows, int newNumberColumns)
{
  ...
  if (newNumberRows != numArtificial_ || newNumberColumns != numStructural_) {
    ...
    if (newNumberColumns > numStructural_ || size > maxSize_) {
      if (size > maxSize_)
        maxSize_ = size + 10;
...
*/
void CoinWarmStartBasis::deleteRows(int rawTgtCnt, const int *rawTgts)
{
  if (rawTgtCnt <= 0)
    return;
...
// Deletes columns
void CoinWarmStartBasis::deleteColumns(int number, const int *which)
{
  int i;
...
void CoinWarmStartBasis::print() const
{
  int i;
  int numberBasic = 0;
  for (i = 0; i < numStructural_; i++) {
...
*/
  const CoinWarmStartBasis *oldBasis = dynamic_cast< const CoinWarmStartBasis * >(oldCWS);
#ifndef NDEBUG
  if (!oldBasis) {
...
*/
  oldStatus = reinterpret_cast< const unsigned int * >(oldBasis->getStructuralStatus());
  newStatus = reinterpret_cast< const unsigned int * >(newBasis->getStructuralStatus());
...
*/
  const CoinWarmStartBasisDiff *diff = dynamic_cast< const CoinWarmStartBasisDiff * >(cwsdDiff);
#ifndef NDEBUG
  if (!diff) {
...
*/
  ...
  unsigned int *structStatus = reinterpret_cast< unsigned int * >(this->getStructuralStatus());
  unsigned int *artifStatus = reinterpret_cast< unsigned int * >(this->getArtificialStatus());
  if (numberChanges >= 0) {
...
CoinWarmStartBasis::Status charToStatus(char status)
{
  ...
  case 'S': {
    return (CoinWarmStartBasis::superBasic);
  }
  case 'X': {
    return (CoinWarmStartBasis::atLowerBound);
  } // fixed
...
CoinWarmStartBasisDiff::CoinWarmStartBasisDiff(const CoinWarmStartBasisDiff &rhs)
  : sze_(rhs.sze_)
  , difference_(0)
{
  if (sze_ > 0) {
